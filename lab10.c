...
void tty_init(void) {
    ...
    GPIOA->MODER &= (~0b11 << (2 * 9)) & (~0b11 << (2 * 10));
    GPIOA->MODER |= (0b10 << (2 * 9)) | (0b10 << (2 * 10));
    GPIOA->AFR[1] &= ~0x00000FF0;
    GPIOA->AFR[1] |= 0x00000100 | 0x00000010;
    RCC->APB2ENR |= RCC_APB2ENR_USART1EN;
    USART1->CR1 &= ~USART_CR1_UE & ~USART_CR1_M & ~USART_CR2_STOP;
    USART1->BRR = 0x1A1;
...
static int getchar_nonirq(void) {
	...
	if ((USART1 ->ISR & USART_ISR_ORE) == USART_ISR_ORE) {
		USART1->ICR |= USART_ICR_ORECF | USART_ICR_ORECF;
	}
    ...
    if (words[0] != 0) {
        ...
        if (strcasecmp(words[0],"display1") == 0) {
        	if (initted_lcd)
				...
				printf("Initialize your lcd first fam\n");
			return;
        }
